...
def determine_possible_extras(whls):
  ...
  def is_possible(distro, extra):
    ...
    for extra_dep in whl.dependencies(extra=extra):
      req = packaging.requirements.Requirement(extra_dep)
      # Check that the dep and any extras are all possible.
      if not is_possible(req.name, None):
        return False
      for e in req.extras:
        if not is_possible(req.name, e):
          return False
...
def main():
  ...
  if subprocess.call(pip_main_argv):
    sys.exit(1)
